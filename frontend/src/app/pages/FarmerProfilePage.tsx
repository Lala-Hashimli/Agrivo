...
const verificationItems = [
  ...
  { label: "Farm location confirmed", icon: MapPin },
  { label: "Product quality checked", icon: ShieldCheck },
  { label: "Delivery handoff supported", icon: Truck },
];
  ...
  return (
    <div className="agrivo-shell agrivo-farmer-profile-page min-h-screen overflow-x-hidden">
      ...
      <div className="agrivo-container py-16 text-center">
        <h1 className="agrivo-heading text-2xl text-[#102018]">Farmer not found</h1>
        <p className="mt-3 text-[#5F6F64]">This profile may have been moved or is no longer available.</p>
        <Button
          ...
          <div className="agrivo-farmer-profile-hero-grid">
            ...
            <div className="agrivo-farmer-profile-hero-content">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-[#5F6F64]">
                ...
                <span className="font-medium text-[#14532D]">{farmer.category}</span>
              </div>
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                <div className="agrivo-farmer-profile-stat">
                  ...
                  <p className="mt-0.5 text-xs text-[#6b7a70]">Rating</p>
                </div>
                <div className="agrivo-farmer-profile-stat">
                  <p className="text-base font-bold text-[#14532D] sm:text-lg">{farmer.completedOrders}</p>
                  <p className="mt-0.5 text-xs text-[#6b7a70]">Completed orders</p>
                </div>
              </div>
    ...
    <div className="agrivo-shell agrivo-farmer-profile-page min-h-screen overflow-x-hidden">
      ...
      <section className="agrivo-farmer-profile-section">
        <div className="agrivo-container max-w-4xl">
          <h2 className="agrivo-heading agrivo-section-title text-[#102018]">About Farmer</h2>
          <p className="mt-4 text-base leading-8 text-[#5F6F64]">{farmer.about}</p>
        </div>
      </section>

      <section className="agrivo-farmer-profile-section bg-[#f8faf4]">
        <div className="agrivo-container">
          <h2 className="agrivo-heading agrivo-section-title mb-5 text-[#102018] sm:mb-6">Farm Details</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {Object.entries({
              "Farm name": farmer.farmDetails.farmName,
              Location: farmer.farmDetails.location,
              "Farm size": farmer.farmDetails.farmSize,
              Experience: farmer.farmDetails.experience,
              "Main products": farmer.farmDetails.mainProducts,
              "Minimum order": farmer.farmDetails.minimumOrder,
              "Delivery support": farmer.farmDetails.deliverySupport,
              "Harvest season": farmer.farmDetails.harvestSeason,
            }).map(([label, value]) => (
              <Card key={label} className="rounded-[24px] border border-[#e5efe1] bg-white">
                <CardContent className="p-4 sm:p-5">
                  <p className="text-sm text-[#5F6F64]">{label}</p>
                  <p className="mt-2 font-semibold text-[#102018]">{value}</p>
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      <section id="farmer-products" className="agrivo-farmer-profile-section agrivo-scroll-anchor">
        <div className="agrivo-container">
          <h2 className="agrivo-heading agrivo-section-title mb-5 text-[#102018] sm:mb-6">Products</h2>
              ...
              <Card key={product.name} className="agrivo-card rounded-[28px] border border-[#e5efe1] bg-white">
                <CardContent className="p-5 sm:p-6">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <Badge className="rounded-full bg-[#ecfdf5] text-[#166534] hover:bg-[#ecfdf5]">{product.category}</Badge>
                      <h3 className="mt-3 text-xl font-semibold text-[#102018]">{product.name}</h3>
                      <p className="mt-2 text-sm leading-7 text-[#5F6F64]">{product.description}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xl font-bold text-[#14532D]">
                        {product.price} <span className="text-sm font-medium">{product.unit}</span>
                      </p>
                      <p className="mt-1 text-sm text-[#5F6F64]">Available: {product.available}</p>
                    </div>
                  </div>
        ...
        <section className="agrivo-farmer-profile-section bg-[#f8faf4]">
          <div className="agrivo-container">
            <div className="mb-5 flex flex-wrap items-end justify-between gap-3 sm:mb-6">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#15803d]">Farm Jobs</p>
                <h2 className="agrivo-heading agrivo-section-title mt-2 text-[#102018]">Active job posts</h2>
              </div>
              <Button
                variant="outline"
                  ...
                  <CardContent className="p-5 sm:p-6">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <h3 className="font-semibold text-[#102018]">{job.title}</h3>
                        <p className="mt-1 flex items-center gap-1.5 text-sm text-[#5F6F64]">
                          <MapPin className="h-3.5 w-3.5 text-[#43A047]" />
                          {job.location}
                        </p>
                        <div className="mt-3 flex flex-wrap gap-3 text-sm">
                          <span className="font-bold text-[#14532D]">{job.dailyPay} AZN / day</span>
                          <span className="text-[#5F6F64]">{formatJobDateRange(job.startDate, job.endDate)}</span>
                        </div>
                      </div>
                    </div>
                  </CardContent>
      ...
      <section className="agrivo-farmer-profile-section bg-[#ecfdf5]">
        <div className="agrivo-container">
          <h2 className="agrivo-heading agrivo-section-title mb-5 text-[#102018] sm:mb-6">Verification</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {verificationItems.map(({ label, icon: Icon }) => (
              <div key={label} className="flex items-center gap-3 rounded-[24px] border border-[#d6f5e3] bg-white p-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-[#102018]">{label}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="agrivo-farmer-profile-section">
        <div className="agrivo-container max-w-4xl">
          <h2 className="agrivo-heading agrivo-section-title mb-5 text-[#102018] sm:mb-6">Buyer Reviews</h2>
          <div className="grid gap-4 md:grid-cols-2">
            {farmer.reviews.map((review) => (
              <Card key={review.author} className="rounded-[24px] border border-[#e5efe1] bg-white">
                <CardContent className="p-5">
                  ...
                  <p className="mt-3 text-sm leading-7 text-[#5F6F64]">{review.text}</p>
                </CardContent>
              </Card>
            ))}
          </div>
              ...
              {otherFarmers.map((item) => (
                <FeaturedFarmerCard
                  ...
                  onViewProfile={(nextSlug) => {
                    window.location.hash = `farmers/${nextSlug}`;
